...
sub initPlugin {
    ...
    Foswiki::Func::registerTagHandler(
        'SKINSTATE',
        sub {
            return Foswiki::Plugins::NatSkinPlugin::ThemeEngine::getThemeEngine(
            )->renderSkinState(@_);
        }
    );
    ...
    Foswiki::Func::registerTagHandler(
        'PREVREV',
        sub {
            return Foswiki::Plugins::NatSkinPlugin::Utils::getPrevRevision(
                $baseWeb, $baseTopic, 1 );
        }
    );
    ...
    Foswiki::Func::registerTagHandler(
        'CURREV',
        sub {
            return Foswiki::Plugins::NatSkinPlugin::Utils::getCurRevision(
                $baseWeb, $baseTopic );
        }
    );
    ...
    Foswiki::Func::registerTagHandler(
        'NATMAXREV',
        sub {
            return Foswiki::Plugins::NatSkinPlugin::Utils::getMaxRevision(
                $baseWeb, $baseTopic );
        }
    );
...
sub postRenderingHandler {
    ...
    $_[0] =~
s/<a\s+([^>]*?href=(?:\"|\'|&quot;)?)([^\"\'\s>]+(?:\"|\'|\s|&quot;>)?)/'<a '.Foswiki::Plugins::NatSkinPlugin::ExternalLink::render($1,$2)/geoi;
}
