...
sub new {
    ...
    %{ $this->{noSideBarActions} } =
      map { $_ => 1 } split( /\s*,\s*/, $noSideBarActions );
...
sub readThemeInfos {
    ...
    my $themePath = Foswiki::Func::getPreferencesValue('NATSKIN_THEMEPATH')
      || Foswiki::Func::getPreferencesValue(
        'STYLEPATH')    # backwards compatibility
...
sub run {
    ...
    if ($request) {
        ...
        if ( $theStyle eq 'reset' ) {
            ...
            my $redirectUrl =
              Foswiki::Plugins::NatSkinPlugin::Utils::getScriptUrlPath('view');
    ...
    $theLayout = $this->{defaultLayout}
      if $theLayout !~ /^(fixed|fluid|bordered)$/;
    ...
    $theStyleSideBar = $this->{defaultStyleSideBar}
    ...
    if ($theStyleVariation) {
        ...
        $theStyleVariation =~ s/\s+$//;
        $doStickyVariation = 1
          if lc($theStyleVariation) ne lc($prefStyleVariation);
    }
    else {
    ...
    foreach my $variation ( keys %{ $themeRecord->{variations} } ) {
        if ( $variation eq $theStyleVariation
            || lc($variation) eq lc($theStyleVariation) )
        {
            $found             = 1;
    ...
    Foswiki::Func::setSessionValue( 'NATSKIN_STYLE',
        $this->{skinState}{'style'} )
      if $doStickyStyle;
    Foswiki::Func::setSessionValue( 'NATSKIN_LAYOUT',
        $this->{skinState}{'layout'} )
      if $doStickyLayout;
    Foswiki::Func::setSessionValue( 'NATSKIN_MENU', $this->{skinState}{'menu'} )
      if $doStickyMenu;
    Foswiki::Func::setSessionValue( 'NATSKIN_SIDEBAR',
        $this->{skinState}{'sidebar'} )
      if $doStickySideBar;
    Foswiki::Func::setSessionValue( 'NATSKIN_VARIATION',
        $this->{skinState}{'variation'} )
    ...
    $theToggleSideBar = 'off'
      if $this->{noSideBarActions}{ $this->{skinState}{'action'} };

    # switch the sidebar off if we need to authenticate
    if ( $this->{noSideBarActions}{login} ) {
        my $authScripts = $Foswiki::cfg{AuthScripts};
        if (
            $this->{skinState}{'action'} ne 'publish'
            &&    # SMELL to please PublishContrib
            $authScripts =~ /\b$this->{skinState}{'action'}\b/
...
# SMELL: better use Trident version to detect the physical version of the browser
    ...
    if ( $skin =~ /\bnat\b/ ) {
        Foswiki::Func::setPreferencesValue( 'FOSWIKI_STYLE_URL',
            '%PUBURLPATH%/%SYSTEMWEB%/NatSkin/BaseStyle.css' );
        Foswiki::Func::setPreferencesValue( 'FOSWIKI_COLORS_URL',
            '%NATSTYLEURL%' );

        Foswiki::Func::addToZone( 'script', 'NATSKIN::JS',
            <<'HERE', 'NATSKIN, NATSKIN::OPTS, JQUERYPLUGIN::FOSWIKI, JQUERYPLUGIN::SUPERFISH, JQUERYPLUGIN::UI' );
...
sub renderSkinStyle {
    ...
    my $theVariation;
    $theVariation = $this->{skinState}{'variation'}
      unless $this->{skinState}{'variation'} =~ /^(off|none)$/;
