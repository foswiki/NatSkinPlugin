...
sub render {
    ...
    $text =
'$edit$sep$attach$sep$new$sep$raw$sep$delete$sep$history$sepprint$sep$more'
      unless defined $text;
    ...
    my $themeEngine =
      Foswiki::Plugins::NatSkinPlugin::ThemeEngine::getThemeEngine();
    ...
    $actionParams->{hiderestricted} =
      Foswiki::Func::isTrue( $params->{hiderestricted}, 0 );
    ...
    %{ $actionParams->{isRestrictedAction} } =
      map { $_ => 1 } split( /\s*,\s*/, $restrictedActions );
    ...
    my $isCompare =
      ( $themeEngine->{skinState}{'action'} eq 'compare' ) ? 1 : 0;
    ...
    # special actions
    $text =~
s/\$(?:editform\b|action\(editform(?:,\s*(.*?))?\))/renderEditForm($actionParams, $1)/ge;
    $text =~
s/\$(?:account\b|action\(account(?:,\s*(.*?))?\))/renderAccount($actionParams, $1)/ge;
    $text =~
s/\$(?:diff\b|action\(diff(?:,\s*(.*?))?\))/renderDiff($actionParams, $1)/ge;
    $text =~
s/\$(?:edit\b|action\(edit(?:,\s*(.*?))?\))/renderEdit($actionParams, $1)/ge;
    $text =~
s/\$(?:first\b|action\(first(?:,\s*(.*?))?\))/renderFirst($actionParams, $1)/ge;
    $text =~
s/\$(?:last\b|action\(last(?:,\s*(.*?))?\))/renderLast($actionParams, $1)/ge;
    $text =~
s/\$(?:login\b|action\(login(?:,\s*(.*?))?\))/renderLogin($actionParams, $1)/ge;
    $text =~
s/\$(?:next\b|action\(next(?:,\s*(.*?))?\))/renderNext($actionParams, $1)/ge;
    $text =~
s/\$(?:prev\b|action\(prev(?:,\s*(.*?))?\))/renderPrev($actionParams, $1)/ge;
    $text =~
      s/\$(?:raw\b|action\(raw(?:,\s*(.*?))?\))/renderRaw($actionParams, $1)/ge;
    $text =~
s/(\$sep)?\$(?:logout\b|action\(logout(?:,\s*(.*?))?\))/renderLogout($actionParams, $1, $2)/ge;

    # normal actions / backwards compatibility
    $text =~
s/\$(attach|copytopic|delete|editsettings|edittext|help|history|more|move|new|pdf|print|register|restore|users)\b/renderAction($1, $actionParams)/ge;

    # generic actions
    $text =~
s/\$action\((.*?)(?:,\s*(.*?))?\)/renderAction($1, $actionParams, undef, undef, $2)/ge;
...
#print STDERR "called renderAction($action,".($context?"'".$context."'":'').")\n";
    ...
    return ''
      if ( defined($context) && !Foswiki::Func::getContext()->{$context} );

    $template = uc($action) . "_ACTION" unless defined $template;
    $restrictedTemplate = uc($action) . "_ACTION_RESTRICTED"
      unless defined $restrictedTemplate;
...
sub getEditUrl {
    ...
    my $url =
      Foswiki::Plugins::NatSkinPlugin::Utils::getScriptUrlPath( "edit", undef,
        undef, 't' => time(), );
...
sub renderMenu {
    ...
    if ( $params->{isRestrictedAction}{'menu'} ) {
        ...
        $menu = Foswiki::Func::expandTemplate('MENU_FORMAT')
          unless defined $menu;
        $header = Foswiki::Func::expandTemplate('MENU_HEADER')
          unless defined $header;
        $footer = Foswiki::Func::expandTemplate('MENU_FOOTER')
...
sub getPdfUrl {
        ...
        || $context->{GenPDFWebkitPluginEnabled} )
    {
        ...
        $url =
          Foswiki::Plugins::NatSkinPlugin::Utils::getScriptUrlPath( 'genpdf',
            undef, undef, 'cover' => 'print', );
    }
...
sub renderEditForm {
    ...
    my ($topicObj) =
      Foswiki::Func::readTopic( $params->{baseWeb}, $params->{baseTopic} );
...
sub getHelpUrl {
    ...
    ( $helpWeb, $helpTopic ) =
      Foswiki::Func::normalizeWebTopicName( $helpWeb, $helpTopic );

    return Foswiki::Plugins::NatSkinPlugin::Utils::getScriptUrlPath( 'view',
        $helpWeb, $helpTopic );
}
...
sub renderFirst {
    ...
    if ( $params->{isRestrictedAction}{'first'}
        || getCurRev($params) == getNrRev($params) + 1 )
    {
...
sub renderLogout {
    ...
    if ($logoutUrl) {
        if ( $params->{isRestrictedAction}{'logout'} ) {
            return '' if $params->{hiderestricted};
            return $sep
              . Foswiki::Func::expandTemplate('LOG_OUT_ACTION_RESTRICTED');
        }
...
sub renderLast {
    ...
    return ( $params->{isRestrictedAction}{'last'}
          || getCurRev($params) == getMaxRev($params) )
...
sub getLastUrl {
    ...
    if ( $params->{action} eq 'view' ) {
        return Foswiki::Plugins::NatSkinPlugin::Utils::getScriptUrlPath( 'view',
            undef, undef, 'rev' => getMaxRev($params) );
    }
    else {
...
sub renderNext {
    ...
    return ( $params->{isRestrictedAction}{'next'}
          || getNextRev($params) > getMaxRev($params) )
...
sub getNextUrl {
    ...
    if ( $params->{action} eq 'view' ) {
        return Foswiki::Plugins::NatSkinPlugin::Utils::getScriptUrlPath( 'view',
            undef, undef, 'rev' => getRev($params) + getNrRev($params) );
    }
    else {
        return Foswiki::Plugins::NatSkinPlugin::Utils::getScriptUrlPath(
            $params->{action}, undef, undef,
...
sub getDiffUrl {
    ...
    if ( $action !~ /^(compare|rdiff)$/ ) {
        my $context = Foswiki::Func::getContext();
        $action =
          $context->{CompareRevisionsAddonPluginEnabled} ? "compare" : "rdiff";
    }
    return Foswiki::Plugins::NatSkinPlugin::Utils::getScriptUrlPath(
        $action, undef, undef,
...
sub getMaxRev {
    ...
    unless ( defined $params->{maxRev} ) {
        $params->{maxRev} =
          Foswiki::Plugins::NatSkinPlugin::Utils::getMaxRevision();
    }
