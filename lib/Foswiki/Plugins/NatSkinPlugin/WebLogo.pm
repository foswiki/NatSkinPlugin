...
sub renderPath {
    ...
    my $themeEngine =
      Foswiki::Plugins::NatSkinPlugin::ThemeEngine::getThemeEngine();
    my $themeRecord =
      $themeEngine->getThemeRecord( $themeEngine->{skinState}{'style'} );
...
sub renderLogo {
    ...
    if ($name) {
        ...
        if ($image) {
            $result =
'<img class="natWebLogo natWebLogoImage" src="$src" alt="$alt" border="0" />';
        }
        else {
            $result =
              '<span class="natWebLogo natWebLogoName">%WIKITOOLNAME%</span>';
        }
    }
