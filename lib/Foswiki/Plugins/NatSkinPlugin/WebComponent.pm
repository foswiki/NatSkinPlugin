...
sub render {
    ...
    return ''
      if $themeEngine->{skinState}{$name}
          && $themeEngine->{skinState}{$name} eq 'off';

    my $text;
    ( $text, $theWeb, $theComponent ) =
      getWebComponent( $session, $theWeb, $theComponent, $theMultiple );
...
#    Although possible, usage of %BASEWEB% in web component topics might have undesired effect when web component is rendered from a foreign web.
    $text =
      Foswiki::Func::expandCommonVariables( $text, $theComponent, $theWeb );
...
sub getWebComponent {
    ...
    if (
        Foswiki::Func::topicExists( $theWeb, $theComponent )
        && Foswiki::Func::checkAccessPermission(
            'VIEW', $userName, undef, $theComponent, $theWeb
        )
      )
    {
          ...
          )
        {
            ...
            # %USERWEB%
            ( $meta, $text ) =
              Foswiki::Func::readTopic( $theWeb, $theComponent );
        }
        else {
            $theWeb = $systemWeb;
